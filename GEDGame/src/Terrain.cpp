...
Terrain::Terrain(void)
    : vertexBuffer(nullptr), indexBuffer(nullptr), diffuseTexture(nullptr),
      diffuseTextureSRV(nullptr), debugSRV(nullptr) {}
...
HRESULT Terrain::create(ID3D11Device *device) {
    ...
    // V(DirectX::CreateDDSTextureFromFile(device, L"resources\\debug_green.dds", nullptr,
    // &debugSRV));

    // if (hr != S_OK) {
    //     MessageBoxA(NULL, "Could not load texture \"resources\\debug_green.dds\"",
    //                 "Invalid texture", MB_ICONERROR | MB_OK);
    //     return hr;
    // }
    ...
    // http://msdn.microsoft.com/en-us/library/ff476899%28v=vs.85%29.aspx
