...
// Workaround to delay initialization as wine segfaults if `GetCursorPos` is called before main()
// for some reason
#define g_camera get_g_camera()
CFirstPersonCamera &get_g_camera() {
    static CFirstPersonCamera cam;
    return cam;
}
...
ID3D11InputLayout *g_terrainVertexLayout; // Describes the structure of the vertex buffer to the
...
int main(void) {
    ...
    DXUTCreateWindow(L"GEDGame"); // You may change the title
...
void InitApp() {
    ...
    iY = 10;
    iY += 24;
    g_sampleUI.AddCheckBox(IDC_TOGGLESPIN, L"Toggle Spinning", 0, iY += 24, 125, 22,
                           g_terrainSpinning);
}
...
HRESULT CALLBACK OnD3D11CreateDevice(ID3D11Device *pd3dDevice,
    ...
    ID3D11DeviceContext *pd3dImmediateContext =
        DXUTGetD3D11DeviceContext(); // http://msdn.microsoft.com/en-us/library/ff476891%28v=vs.85%29
    ...
    const D3D11_INPUT_ELEMENT_DESC
        layout[] = // http://msdn.microsoft.com/en-us/library/bb205117%28v=vs.85%29.aspx
        {
            {"SV_POSITION", 0, DXGI_FORMAT_R32G32B32A32_FLOAT, 0, 0, D3D11_INPUT_PER_VERTEX_DATA,
             0},
...
HRESULT CALLBACK OnD3D11ResizedSwapChain(ID3D11Device *pd3dDevice, IDXGISwapChain *pSwapChain,
    ...
    g_camera.SetProjParams(g_cameraParams.fovy, g_cameraParams.aspect, g_cameraParams.nearPlane,
                           g_cameraParams.farPlane);
...
LRESULT CALLBACK MsgProc(HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam,
    ...
    *pbNoFurtherProcessing = g_sampleUI.MsgProc(hWnd, uMsg, wParam, lParam);
    ...
    if (uMsg == WM_MOUSEWHEEL) {
        ...
        g_cameraMoveScaler *= (1 + (float)zDelta / 500.0f);
...
void CALLBACK OnFrameMove(double fTime, float fElapsedTime, void *pUserContext) {
    ...
    if (g_terrainSpinning) {
        ...
        g_terrainWorld *=
            XMMatrixRotationY(30.0f * DEG2RAD((float)fTime)); // Rotate around world-space "up" axis
    }
